...
export default function ServicesCatalog() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-gray-50 scroll-smooth overflow-hidden">
      ...
      {/* HERO SECTION */}
      <section className="w-full pt-20 md:pt-24 pb-12 md:pb-16 px-6 md:px-12 bg-white text-center">
        <div className="max-w-4xl mx-auto">
          <div className="inline-block px-4 py-1.5 bg-secondary/20 rounded-full font-nunito text-xs font-bold tracking-widest mb-4 md:mb-6 text-secondary border border-secondary/30 uppercase">
          ...
          <p className="font-nunito text-base md:text-lg text-neutral-dark max-w-2xl mx-auto text-balance">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {displayServices.map((s: any, i: number) => (
              <div key={i} className="bg-white rounded-[24px] md:rounded-[30px] shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition duration-500 group flex flex-col">
                ...
                <div className="p-6 md:p-8 flex flex-col flex-grow">
                  <h3 className="font-poppins text-xl md:text-2xl font-bold text-primary mb-3 md:mb-4">{getTxt(s.title)}</h3>
                  <p className="font-nunito text-sm md:text-base text-neutral-dark mb-6 md:mb-8 leading-relaxed flex-grow">{getTxt(s.description)}</p>
                  <Link href={`/layanan/${s.slug?.[lang]?.current || s.slug?.id?.current}`} className="text-secondary font-bold text-xs md:text-sm uppercase tracking-wider hover:text-primary transition inline-flex items-center">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          <div>
            <h2 className="font-poppins text-3xl md:text-4xl lg:text-5xl font-bold text-primary mb-4 md:mb-8 text-balance">
              {getTxt(settingsData?.kontakTitle, "Siap Berakselerasi?")}
            </h2>
            <p className="font-nunito text-base md:text-lg text-neutral-dark mb-8 md:mb-12">
              {getTxt(settingsData?.kontakDesc, "Jadilah mitra pertumbuhan digital kami.")}
            </p>
            
            <div className="space-y-6 md:space-y-8 font-nunito font-semibold text-neutral-dark">
              <div className="flex items-center group">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-primary/5 rounded-xl flex items-center justify-center mr-4 md:mr-6 group-hover:bg-primary transition flex-shrink-0">
                  <svg className="w-5 h-5 md:w-6 md:h-6 text-primary group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                </div>
                <span className="text-sm md:text-base leading-snug">{settingsData?.officeAddress || "Denpasar, Bali - Indonesia"}</span>
              </div>
              <div className="flex items-center group">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-primary/5 rounded-xl flex items-center justify-center mr-4 md:mr-6 group-hover:bg-primary transition flex-shrink-0">
                  <svg className="w-5 h-5 md:w-6 md:h-6 text-primary group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                </div>
