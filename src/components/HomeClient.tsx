...
// Komponen ini menerima data "matang" dari Server (page.tsx)
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="bg-white p-8 md:p-12 rounded-[30px] md:rounded-[40px] shadow-2xl border border-gray-100">
            <form onSubmit={handleContactSubmit} className="space-y-5 md:space-y-6 font-nunito">
              ...
              <button disabled={status === 'loading'} type="submit" className={`w-full text-white font-poppins font-bold py-4 md:py-5 rounded-2xl transition text-base md:text-lg shadow-lg ${status === 'loading' ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-primary-light'}`}>
                {status === 'loading' ? (lang === 'id' ? 'Mengirim...' : 'Sending...') : (lang === 'id' ? 'Kirim Pesan' : 'Send Message')}
              </button>

              {status === 'success' && <p className="text-green-600 font-bold text-center mt-4 text-sm bg-green-50 p-3 rounded-lg border border-green-200 animate-fade-in">{lang === 'id' ? '✅ Pesan berhasil terkirim! Tim kami akan segera menghubungi Anda.' : '✅ Message sent successfully! Our team will contact you soon.'}</p>}
              {status === 'error' && <p className="text-red-600 font-bold text-center mt-4 text-sm bg-red-50 p-3 rounded-lg border border-red-200 animate-fade-in">{lang === 'id' ? '❌ Gagal mengirim pesan. Silakan coba lagi.' : '❌ Failed to send message. Please try again.'}</p>}
            </form>
          </div>
        </div>
      </section>
